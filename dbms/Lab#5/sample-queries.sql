...
WHERE Person_ID IN (
        ...
        WHERE T1.Student_ID = T2.Student_ID
            AND T2.Dept_Name = 'Computer Science Engineering'
    );
-- List all the Equipment belonging to a particular Lab.
SELECT *
FROM Equipment1
WHERE Lab_Name = 'Network Lab'
    AND Dept_Name = 'Computer Science Engineering';
...
SELECT *
FROM Comments
WHERE Topic LIKE '%cholarship%'
    AND Stakeholder_Person_ID IN (
        SELECT Person_ID
        FROM Comments as C,
            Stakeholder as S
        WHERE C.Stakeholder_Person_ID = S.Person_ID
            AND S.Domain = 'Individual'
    );